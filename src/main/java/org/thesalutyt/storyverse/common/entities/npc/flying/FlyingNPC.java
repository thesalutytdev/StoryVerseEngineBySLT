...
public class FlyingNPC extends FlyingEntity implements IAnimatable, IAnimationTickable, IMerchant {
    private int ticks = 0;
    ...
    private static final DataParameter<Boolean> SLEEP =
            EntityDataManager.defineId(FlyingNPC.class, DataSerializers.BOOLEAN);

    private static final DataParameter<String> ANIMATION =
            EntityDataManager.defineId(FlyingNPC.class, DataSerializers.STRING);
    ...
    protected FlyingNPC(EntityType<? extends FlyingEntity> npc, World level) {
        super(npc, level);
    }
...
//            yBodyRotO = rotY.floatValue();
    ...
    public void hold(Hand hand, ItemStack item) {
        ...
        FlyingNPCRender.renders.get(0).hold(hand, item);
    }
    ...
    public void armor(Integer slot, ItemStack item) {
        ...
        FlyingNPCRender.renders.get(0).armor(slot, item);
    }
