package org.thesalutyt.storyverse.forge.api.environment.js;
...
import org.thesalutyt.storyverse.forge.api.environment.js.interpreter.EventLoop;
...
//        }
        ...
        } else if (event.getEntity().isCrouching()) {
    ...
    public static void onKilled(LivingDeathEvent event) {
        ...
        if (!Objects.equals(event.getSource(), DamageSource.playerAttack(Player.getPlayer()))) {
            System.out.printf("%s(%s, %s) killed %s with this damage source: %s%n",
                    event.getEntity().getType(), event.getEntity().getUUID(),
                    event.getEntity(), event.getSource().getEntity(),
                    event.getSource());
            return;
        }
        if (entity instanceof LivingEntity) {
            System.out.println("Player killed " + entity.getUUID() + " (" + event.getEntity().getType() + ")");
            runEvent(getMob(entity.getUUID()), "kill");
        }
    }
    ...
    public static MobController npc(String id, Double x, Double y, Double z, String name, Boolean visible,
        ...
        EntityData entityData_ = new EntityData((LivingEntity) npc.self(), name, x, y, z,
                new Object[]{npc.getModelPath(), npc.getAnimation(), npc.getTexturePath()});
